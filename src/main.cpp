...
#if defined(NDEBUG)
# error "FungiX cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0b8598fe8e178ee5629f664003bb888e4ec63f9ec1ccaef2a945c884f79a7939");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // FungiX: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "FungiX Signed Message:\n";
...
{
    // FungiX: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 880000); // FungiX: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // FungiX: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // FungiX: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // FungiX: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // FungiX: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc8397ca10e720b9dbd17b646f5331dae7a08b2a89a1ef12fb22064dc4ef44243");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "FungiX was born on 27th of Sept 2022";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0437ba3a8368c2cd3cc9b0dc59ef870969d5e0952e56e9db09833db2b299f93bd0e8ba006611d3c5c41a85eac15f7f82c5df9b0d672d36e3fb81b45f9f893b1d6d") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1664282115;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2088554413;

        if (fTestNet)
        {
            block.nTime    = 1664282094;
            block.nNonce   = 385278895;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xf4bc0c749ec814aac7106333a93246ecb77c40929029f570bdc7667c51fb24f4"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc6, 0xba, 0xd3 }; // FungiX: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FungiXMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FungiXMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FungiXMiner(CWallet *pwallet)
{
    printf("FungiXMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("fungix-miner");
    ...
    try { loop {
        ...
        printf("Running FungiXMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FungiXMiner, pwallet));
}
